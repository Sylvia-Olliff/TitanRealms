...
public class GuiConfirmationDialog extends GuiWindow {
    ...
    public GuiConfirmationDialog(IGuiWrapper gui, int x, int y, int width, int height, ITextComponent title, Runnable onConfirm, DialogType type) {
        ...
        addChild(new TranslationButton(gui, this.x + width / 2 - 102 / 2, this.y + height - 24, 50, 18, TitanRealmsLang.BUTTON_CANCEL, this::close));
        addChild(new TranslationButton(gui, this.x + width / 2 + 1, this.y + height - 24, 50, 18, TitanRealmsLang.BUTTON_CONFIRM, () -> {
            onConfirm.run();
            close();
