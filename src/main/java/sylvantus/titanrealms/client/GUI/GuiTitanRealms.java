...
import sylvantus.titanrealms.client.GUI.element.GuiElement.IHoverable;
import sylvantus.titanrealms.client.GUI.element.GuiWindow;
import sylvantus.titanrealms.client.GUI.element.slot.GuiSlot;
import sylvantus.titanrealms.client.GUI.element.slot.SlotType;
import sylvantus.titanrealms.core.util.interfaces.client.IGuiWrapper;
import sylvantus.titanrealms.core.util.interfaces.render.IFancyFontRenderer;
import sylvantus.titanrealms.client.render.TitanRealmsRenderer;
import sylvantus.titanrealms.common.containers.slot.ContainerSlotType;
import sylvantus.titanrealms.common.containers.slot.InventoryContainerSlot;
import sylvantus.titanrealms.common.containers.slot.SlotOverlay;
import sylvantus.titanrealms.core.lib.LRU;
...
public abstract class GuiTitanRealms<CONTAINER extends Container> extends ContainerScreen<CONTAINER> implements IGuiWrapper, IFancyFontRenderer {

    private static final NumberFormat intFormatter = NumberFormat.getIntegerInstance();
    public static final ResourceLocation BASE_BACKGROUND = TitanRealmsUtils.getResource(ResourceType.GUI, "base.png");
    public static final ResourceLocation SHADOW = TitanRealmsUtils.getResource(ResourceType.GUI, "shadow.png");
    public static final ResourceLocation BLUR = TitanRealmsUtils.getResource(ResourceType.GUI, "blur.png");
    ...
    protected GuiTitanRealms(CONTAINER container, PlayerInventory inv, ITextComponent title) {
    ...
    protected ResourceLocation getButtonLocation(String name) {
        return TitanRealmsUtils.getResource(ResourceType.GUI_BUTTON, name + ".png");
    }
    ...
    protected void drawGuiContainerForegroundLayer(@Nonnull MatrixStack matrix, int mouseX, int mouseY) {
        ...
        children().stream().filter(c -> c instanceof GuiElement).forEach(c -> ((GuiElement) c).onDrawBackground(matrix, mouseX, mouseY, TitanRealmsRenderer.getPartialTick()));
        RenderSystem.translatef(guiLeft, guiTop, 0);
...
//                } else
                if (slotType == ContainerSlotType.INPUT || slotType == ContainerSlotType.OUTPUT || slotType == ContainerSlotType.EXTRA) {
...
//        if (container instanceof TitanRealmsTileContainer) {
//            TileEntityTitanRealms tileEntity = ((TitanRealmsTileContainer<?>) container).getTileEntity();
...
//    }
    ...
    protected void drawGuiContainerBackgroundLayer(@Nonnull MatrixStack matrix, float partialTick, int mouseX, int mouseY) {
        ...
        if (width < 8 || height < 8) {
            TitanRealms.LOGGER.warn("Gui: {}, was too small to draw the background of. Unable to draw a background for a gui smaller than 8 by 8.", getClass().getSimpleName());
            return;
        }
