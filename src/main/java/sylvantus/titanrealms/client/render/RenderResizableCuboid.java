...
import sylvantus.titanrealms.client.render.TitanRealmsRenderer.Model3D;
...
public class RenderResizableCuboid {
    ...
    public void renderCube(Model3D cube, MatrixStack matrix, IVertexBuilder buffer, int argb, int light, int overlay) {
        float red = TitanRealmsRenderer.getRed(argb);
        float green = TitanRealmsRenderer.getGreen(argb);
        float blue = TitanRealmsRenderer.getBlue(argb);
        float alpha = TitanRealmsRenderer.getAlpha(argb);
