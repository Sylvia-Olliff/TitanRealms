package sylvantus.titanrealms.client.render.lib;
...
public class ColorAtlas {
    ...
    public void parse(ResourceLocation rl) {
        ...
        if (parsed.size() < colors.size()) {
            TitanRealms.LOGGER.error("Failed to parse '{}' color atlas.", name);
            return;
        }
    ...
    public static List<Color> load(ResourceLocation rl, int count) {
        ...
        try {
            ...
            TitanRealms.LOGGER.error("Failed to load color atlas: {}", rl, e);
            e.printStackTrace();
        }
        return ret;
    }
    ...
    private static void loadColorAtlas(ResourceLocation rl, int count, List<Color> ret) throws IOException {
        ...
        for (int i = 0; i < count; i++) {
            ...
            if (rgb >> 24 == 0) {
                ...
                ret.add(Color.WHITE);
                TitanRealms.LOGGER.warn("Unable to retrieve color marker: '{}' for atlas: '{}'. This is likely due to an out of date resource pack.", count, rl);
            } else {
