package sylvantus.titanrealms.client.sound;
...
import sylvantus.titanrealms.client.sound.PlayerSound.SoundType;
...
public class SoundHandler {
    ...
    public static void startSound(@Nonnull IWorld world, @Nonnull UUID uuid, @Nonnull SoundType soundType) {
        switch (soundType) {
            case DEBUG:
//                if (!exampleSounds.contains(uuid)) {
//                    PlayerEntity player = world.getPlayerByUuid(uuid);
//                    if (player != null) {
//                        exampleSounds.add(uuid);
//                        playSound(new ExampleSound(player));
//                    }
//                }
                break;
        }
    ...
    public static void playSound(SoundEvent sound) {
        playSound(SimpleSound.master(sound, 1, TitanRealmsConfig.client.baseSoundVolume.get()));
    }
    ...
    public static void onTilePlaySound(PlaySoundEvent event) {
        ...
        if (resultSound == null || IS_MUFFLED_CHECK) {
            return;
        }
        ...
        if (!soundLoc.getNamespace().startsWith(TitanRealms.MODID)) {
            return;
        }
    ...
    private static class TileTickableSound extends TickableSound {
        ...
        TileTickableSound(SoundEvent soundEvent, SoundCategory category, BlockPos pos, float volume) {
            ...
            this.originalVolume = volume * TitanRealmsConfig.client.baseSoundVolume.get();
