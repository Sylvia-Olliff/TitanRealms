...
import sylvantus.titanrealms.core.lib.math.voxel.VoxelCuboid.WallRelative;
...
import sylvantus.titanrealms.core.lib.multiblock.FormationProtocol.CasingType;
import sylvantus.titanrealms.core.util.WorldUtils;
import sylvantus.titanrealms.core.util.interfaces.multiblock.IMultiblock;
import sylvantus.titanrealms.core.util.interfaces.multiblock.IMultiblockBase;
import sylvantus.titanrealms.core.util.interfaces.multiblock.IStructuralMultiblock;
...
public abstract class CuboidStructureValidator<T extends MultiblockData> implements IStructureValidator<T> {
    ...
    protected FormationResult validateNode(FormationProtocol<T> ctx, Long2ObjectMap<IChunk> chunkMap, BlockPos pos) {
        ...
        if (requirement.isCasing()) {
            ...
            return FormationResult.fail(TitanRealmsLang.MULTIBLOCK_INVALID_INNER, pos);
    ...
    protected FormationResult validateFrame(FormationProtocol<T> ctx, BlockPos pos, BlockState state, CasingType type, boolean needsFrame) {
        ...
        if (!isFrameCompatible((TileEntity) tile) || (needsFrame && !type.isFrame())) {
            ...
            return FormationResult.fail(TitanRealmsLang.MULTIBLOCK_INVALID_FRAME, pos);
        } else {
