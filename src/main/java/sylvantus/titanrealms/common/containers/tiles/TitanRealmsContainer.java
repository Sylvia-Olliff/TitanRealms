...
import sylvantus.titanrealms.common.containers.slot.*;
...
import sylvantus.titanrealms.core.network.sync.list.SyncableList;
...
import sylvantus.titanrealms.core.network.sync.ISyncableData.DirtyType;
...
public abstract class TitanRealmsContainer extends Container {
    ...
    protected TitanRealmsContainer(ContainerTypeRegistryObject<?> type, int id, @Nullable PlayerInventory inv) {
    ...
    private <MSG> void sendChange(MSG packet) {
        for (IContainerListener listener : listeners) {
            if (listener instanceof ServerPlayerEntity) {
                TitanRealms.packetHandler.sendTo(packet, (ServerPlayerEntity) listener);
            }
        }
    }
    ...
    public void addListener(@Nonnull IContainerListener listener) {
        ...
        if (!alreadyHas && listener instanceof ServerPlayerEntity) {
            ...
            if (size == 1) {
                //If we only have a single element send a type specific packet to reduce overhead of
                // having to include type and count
                TitanRealms.packetHandler.sendTo(dirtyData.get(0).getSinglePacket((short) windowId), (ServerPlayerEntity) listener);
            } else if (size > 1) {
                TitanRealms.packetHandler.sendTo(new PacketUpdateContainerBatch((short) windowId, dirtyData), (ServerPlayerEntity) listener);
            }
        }
    }
