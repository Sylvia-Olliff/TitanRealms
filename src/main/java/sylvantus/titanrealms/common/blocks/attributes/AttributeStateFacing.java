...
public class AttributeStateFacing implements IAttributeState {

    private final DirectionProperty facingProperty;
    private final FacePlacementType placementType;
    private final boolean canRotate;

    public AttributeStateFacing() { this(true); }
    ...
    public BlockState copyStateData(BlockState oldState, BlockState newState) {
        if (IAttribute.has(newState.getBlock(), AttributeStateFacing.class)) {
            DirectionProperty oldFacingProperty = IAttribute.get(oldState.getBlock(), AttributeStateFacing.class).getFacingProperty();
            newState = newState.with(IAttribute.get(newState.getBlock(), AttributeStateFacing.class).getFacingProperty(), oldState.get(oldFacingProperty));
        }
        return newState;
    }
    ...
    public BlockState getStateForPlacement(Block block, @Nullable BlockState state, @Nonnull IWorld world, @Nonnull BlockPos pos, @Nullable PlayerEntity player,
        ...
        AttributeStateFacing blockFacing = IAttribute.get(block, AttributeStateFacing.class);
    ...
    public static BlockState rotate(BlockState state, Rotation rotation) {
        ...
        if (IAttribute.has(block, AttributeStateFacing.class)) {
            AttributeStateFacing blockFacing = IAttribute.get(block, AttributeStateFacing.class);
